...
-   Example: $X_1, X_2, \cdots, X_n \iid \mathcal{N}(\mu, \sigma^2)$ with $\sigma^2$ known; then _µ_, the unknown mean, is treated as fixed
    -   To account for variability across samples, we develop [**estimators**]{.alert} (e.g. $\bar{X}$) to estimate the "true value" of _µ_.
    
-   The Bayesian framework acknowledges that we are coming into our problem with certain _prior beliefs_, and, consequently, treats _µ_ as a _random variable in itself_, following some prespecified [**prior distribution**]{.alert}.
...
m_0.1 <- data.frame(x = 0:3) %>%
  ...
  ggtitle("Matérn-5/2; h = 0.1") + xlab("r") +
...
m_0.5 <- data.frame(x = 0:3) %>%
  ...
  theme_minimal(base_size = 18) +
  labs(colour = bquote(nu)) +
  ggtitle("Matérn-5/2; h = 0.5") + xlab("r") +
    theme(plot.title = element_text(face = "bold"))
```
...
-   **Prior:** $f \sim \GP(0, \ \tau^2 C(r))$ where $C(r)$ denotes the [**Matérn-5/2 Kernel**]{.alert}
...
::: {.nonincremental}
-   **Prior:** $f \sim \GP$(0, [𝜏<sup>2</sup>]{.bg style="--col: #ffb000"}_C_(_r_)) where _C_(_r_) denotes the [**Matérn-5/2 Kernel**]{.alert}
$$ C(r) = \left( 1 + \frac{r \sqrt{5}}{\colorbox{#ffb000}{𝘩}} + \frac{5r^2}{3\colorbox{#ffb000}{𝘩}^2} \right) \exp\left\{ - \frac{r \sqrt{5}}{\colorbox{#ffb000}{𝘩}} \right\}  $$

-   **Noise:** [**White Noise**]{.alert} process $\omega_t \iid \mathcal{N}$(0, [σ<sup>2</sup>]{.bg style="--col: #ffb000"})
:::
...
-   Essentially, most functions can be decomposed into a series of sines and cosines.
    -   The coefficients of these sines and cosines (which are essentially _coordinates_) are called the [**Fourier Coefficients**]{.alert} of the function.

-   For example, the the [**Sawtooth Wave**]{.alert} $$f(x) = x - 2 \cdot \mathrm{round}(x / 2)$$ admits the following Fourier decomposition:
...
-   **Time-Domain Model:** $y_t = f(t) + \omega_t$
    ...
    -   $\omega_t \iid \mathcal{N}(0, \ \sigma^2)$
:::
:::
...
to obtain estimates $\widehat{\eta}_k := \widehat{w} \check{y}_k$.
    
:::

5)    Apply the inverse Fourier transform to the $\widehat{\eta}_k$ values from step (4) above to obtain fitted values $\hvect{y}$.
...
-   Admittedly, this induces some heteroskedasticity which must be accounted for.
    ...
    -   A potential future direction of research that is of interest to us is finding a more systematic way to handle this heteroskedasticity.
